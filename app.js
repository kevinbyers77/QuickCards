/* Enhanced UI + brightness & haptics */
(() => {
  ...
  function savePrefs(){
    localStorage.setItem(PREFS_KEY, JSON.stringify({bright:!!brightToggle.checked,haptics:!!hapticsToggle.checked}));
  }
  ...
  async function enterBrightMode(){
    ...
    try{ const el = document.documentElement; if (!document.fullscreenElement && el.requestFullscreen) await el.requestFullscreen({navigationUI:'hide'});}catch{}
  }
  ...
  function openDB(){
    return new Promise((resolve,reject)=>{
      ...
      req.onupgradeneeded = () => {
        ...
        store.createIndex('name','name'); store.createIndex('lastOpened','lastOpened'); store.createIndex('openCount','openCount');
      };
      req.onsuccess = () => resolve(req.result); req.onerror = () => reject(req.error);
    });
  }
  async function idb(mode){ if(!db) db = await openDB(); return db.transaction(storeName,mode).objectStore(storeName); }
  async function getAll(){ const s = await idb('readonly'); return new Promise((res,rej)=>{ const it=[]; const r=s.openCursor(); r.onsuccess=e=>{const c=e.target.result; if(c){it.push(c.value); c.continue();} else res(it)}; r.onerror=()=>rej(r.error)}); }
  ...
  const blobFrom = (file,type) => new Blob([file], { type });
  ...
  async function render(){
    ...
    for (const card of cards){
      ...
      imgEl.src = URL.createObjectURL(card.cardImage); imgEl.onload = () => URL.revokeObjectURL(imgEl.src);
      ...
      node.querySelector('.thumbBtn').addEventListener('click', async () => {
        ...
        card.openCount = (card.openCount||0)+1; card.lastOpened = Date.now(); await putCard(card); await render();
      });
      node.querySelector('.edit').addEventListener('click', ()=> openEdit(card));
      node.querySelector('.del').addEventListener('click', async ()=>{ if(confirm(`Delete “${card.name}”?`)){ await deleteCard(card.id); await render(); } });
  ...
  function openEdit(card){
    ...
    if (card){ editForm.dataset.id = card.id; document.getElementById('nameInput').value = card.name||''; }
  ...
  editForm.addEventListener('submit', async (e)=>{
    ...
    if (cardImageInput.files && cardImageInput.files[0]){
      ...
      cardImageBlob = blobFrom(new Blob([arr]), cardImageInput.files[0].type || 'image/png');
    }
    if (barcodeImageInput.files && barcodeImageInput.files[0]){
      const arr2 = await readFile(barcodeImageInput.files[0]);
      barcodeImageBlob = blobFrom(new Blob([arr2]), barcodeImageInput.files[0].type || 'image/png');
    }

    if (id){
      const current = (await getAll()).find(c => c.id === id);
      const updated = {...current, name, cardImage: cardImageBlob||current.cardImage, barcodeImage: barcodeImageBlob||current.barcodeImage};
